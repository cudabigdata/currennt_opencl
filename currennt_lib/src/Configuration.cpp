...
Configuration::Configuration(int argc, const char *argv[])
{
    ...
    commonOptions.add_options()
        ...
        ("opencl",             po::value(&m_useOpenCL)        ->default_value(false),         "use OpenCL to accelerate the computations")
    ...
    if (m_useOpenCL){
    	std::cout << "Utilizing the GPU (OpenCL) for computations with " << m_parallelSequences << " sequences in parallel." << std::endl;
    	// If OpenCL mode enable, must disable default CUDA mode
    	m_useCuda = false;
    }
...
bool Configuration::useOpenCL() const
{
    return m_useOpenCL;
}
