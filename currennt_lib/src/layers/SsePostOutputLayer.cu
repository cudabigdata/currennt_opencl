...
namespace layers {
    ...
    real_t SsePostOutputLayer<TDevice>::calculateError()
    {
		...
		int offset = 0;
		int size =  this->patTypes().size() > 10 + offset ? 10 + offset : this->patTypes().size();
		for (int i = offset; i < size ; i ++)
			std::cout <<  this->patTypes()[i] << " ";
		std::cout << std::endl;
   //    n = 50000;
...
//	int offset =0;
//	int size =  this->_outputErrors().size() > 10 + offset ? 10 + offset :   this->_outputErrors().size();
//	for (int i = offset; i < size ; i ++)
//		std::cout <<    this->_outputErrors()[i] << " ";
//	std::cout << std::endl;
    }
