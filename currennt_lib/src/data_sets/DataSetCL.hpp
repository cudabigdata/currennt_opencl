...
#ifndef DATA_SETS_DATASETCL_HPP
#define DATA_SETS_DATASETCL_HPP
...
namespace data_sets {
     ...
     * to a file.
     *********************************************************************************************/
    class DataSetCL : boost::noncopyable
    {
    public:
        struct sequence_t {
            int         originalSeqIdx;
    ...
    private:
        ...
        void _addNoise(std::vector<real_t> *v);
        std::vector<real_t> _loadInputsFromCache(const sequence_t &seq);
        std::vector<real_t> _loadOutputsFromCache(const sequence_t &seq);
        std::vector<int> _loadTargetClassesFromCache(const sequence_t &seq);
        boost::shared_ptr<DataSetFractionCL> _makeFractionTask(int firstSeqIdx);
        boost::shared_ptr<DataSetFractionCL> _makeFirstFractionTask();
    ...
    private:
        ...
        std::vector<real_t> m_outputMeans;
        std::vector<real_t> m_outputStdevs;
    ...
    public:
         ...
         */
        DataSetCL(const std::vector<std::string> &ncfiles, int parSeq, real_t fraction=1,
         ...
         */
        boost::shared_ptr<DataSetFractionCL> getNextFraction();
         ...
         */
        std::vector<real_t> outputMeans() const;
         ...
         */
        std::vector<real_t> outputStdevs() const;
