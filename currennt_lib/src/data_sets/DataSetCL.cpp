...
namespace internal {
    ...
    template <typename T>
    std::vector<T> readNcArray(int ncid, const char *arrName, int begin, int n)
    {
        int ret;
        int varid;
        size_t start[] = {begin};
    ...
    std::vector<real_t> readNcPatternArray(int ncid, const char *arrName, int begin, int n, int patternSize)
    {
        int ret;
        int varid;
        ...
        std::vector<real_t> v(n * patternSize);
    ...
    std::vector<real_t> targetClassesToOutputs(const std::vector<int> &targetClasses, int numLabels)
    {
        if (numLabels == 2) {
            std::vector<real_t> v(targetClasses.size());
            ...
            std::vector<real_t> v(targetClasses.size() * numLabels, 0);
    ...
    bool comp_seqs(const data_sets::DataSetCL::sequence_t &a, const data_sets::DataSetCL::sequence_t &b)
    {
...
namespace data_sets {
    ...
    struct thread_data_t
    {
        ...
        boost::function<boost::shared_ptr<DataSetFractionCL> ()> taskFn;
        boost::shared_ptr<DataSetFractionCL> frac;
        bool finished;
    };

    void DataSetCL::_nextFracThreadFn()
    {
        for (;;) {
            // wait for a new task
    ...
    void DataSetCL::_addNoise(std::vector<real_t> *v)
    {
        if (!m_noiseDeviation)
            return;
    ...
    std::vector<real_t> DataSetCL::_loadInputsFromCache(const sequence_t &seq)
    {
    	std::vector<real_t> v(seq.length * m_inputPatternSize);
    ...
    std::vector<real_t> DataSetCL::_loadOutputsFromCache(const sequence_t &seq)
    {
    	std::vector<real_t> v(seq.length * m_outputPatternSize);
    ...
    std::vector<int> DataSetCL::_loadTargetClassesFromCache(const sequence_t &seq)
    {
    	std::vector<int> v(seq.length);
    ...
    boost::shared_ptr<DataSetFractionCL> DataSetCL::_makeFractionTask(int firstSeqIdx)
    {
        ...
        boost::shared_ptr<DataSetFractionCL> frac(new DataSetFractionCL);
        ...
        for (int seqIdx = firstSeqIdx; seqIdx < firstSeqIdx + m_parallelSequences; ++seqIdx) {
            if (seqIdx < (int)m_sequences.size()) {
                ...
                DataSetFractionCL::seq_info_t seqInfo;
        ...
        for (int i = 0; i < m_parallelSequences; ++i) {
            ...
            // inputs
            std::vector<real_t> inputs = _loadInputsFromCache(seq);
            _addNoise(&inputs);
            ...
            // target classes
            if (m_isClassificationData) {
                std::vector<int> targetClasses = _loadTargetClassesFromCache(seq);
                for (int timestep = 0; timestep < seq.length; ++timestep) {
            ...
            // outputs
                ...
                std::vector<real_t> outputs = _loadOutputsFromCache(seq);
                for (int timestep = 0; timestep < seq.length; ++timestep) {
    ...
    boost::shared_ptr<DataSetFractionCL> DataSetCL::_makeFirstFractionTask()
    {
    ...
    DataSetCL::DataSetCL(const std::vector<std::string> &ncfiles, int parSeq, real_t fraction, int truncSeqLength, bool fracShuf, bool seqShuf, real_t noiseDev, std::string cachePath)
            ...
            nc_itr != ncfiles.end(); ++nc_itr) 
        {
            ...
            try {
                ...
                for (std::vector<sequence_t>::iterator seq = sequences.begin(); seq != sequences.end(); ++seq) {
                    ...
                    std::vector<real_t> inputs = internal::readNcPatternArray(ncid, "inputs", inputsBegin, seq->length, m_inputPatternSize);
                    ...
                    if (m_isClassificationData) {
                        std::vector<int> targets = internal::readNcArray<int>(ncid, "targetClasses", targetsBegin, seq->length);
                        ...
                        std::vector<real_t> targets = internal::readNcPatternArray(ncid, "targetPatterns", targetsBegin, seq->length, m_outputPatternSize);
                ...
                if (first_file) {
                    ...
                    try {
                        ...
                        m_outputMeans  = std::vector<real_t>(m_outputPatternSize, 0.0f);
                        m_outputStdevs = std::vector<real_t>(m_outputPatternSize, 1.0f);
                    }
                }
                ...
                m_threadData->thread    = boost::thread(&DataSetCL::_nextFracThreadFn, this);
            }
    ...
    bool DataSetCL::isClassificationData() const
    {
        return m_isClassificationData;
    }

    bool DataSetCL::empty() const
    {
        return (m_totalTimesteps == 0);
    }

    boost::shared_ptr<DataSetFractionCL> DataSetCL::getNextFraction()
    {
        // initial work
        if (m_curFirstSeqIdx == -1) {
            boost::unique_lock<boost::mutex> lock(m_threadData->mutex);
            m_threadData->taskFn = boost::bind(&DataSetCL::_makeFirstFractionTask, this);
            m_threadData->finished = false;
            m_threadData->cv.notify_one();
            m_curFirstSeqIdx = 0;
        }
        ...
        if (m_curFirstSeqIdx < (int)m_sequences.size()) {
            ...
            // start new task
            if (m_curFirstSeqIdx < (int)m_sequences.size())
                m_threadData->taskFn = boost::bind(&DataSetCL::_makeFractionTask, this, m_curFirstSeqIdx);
            else
                m_threadData->taskFn = boost::bind(&DataSetCL::_makeFirstFractionTask, this);
    ...
    int DataSetCL::totalSequences() const
    {
        return m_totalSequences;
    }

    int DataSetCL::totalTimesteps() const
    {
        return m_totalTimesteps;
    }

    int DataSetCL::minSeqLength() const
    {
        return m_minSeqLength;
    }

    int DataSetCL::maxSeqLength() const
    {
        return m_maxSeqLength;
    }

    int DataSetCL::inputPatternSize() const
    {
        return m_inputPatternSize;
    }

    int DataSetCL::outputPatternSize() const
    {
        return m_outputPatternSize;
    }

    std::vector<real_t> DataSetCL::outputMeans() const
    {
        return m_outputMeans;
    }

    std::vector<real_t> DataSetCL::outputStdevs() const
    {
        return m_outputStdevs;
    }

    std::string DataSetCL::cacheFileName() const
    {
